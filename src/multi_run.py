...
def main(rank: int, world_size: int, save_every: int, total_epochs: int, batch_size: int):
    ...
    if(rank==0):
        ...
        output_path = f"outputs/{world_size}model_state_dict.pth"
        if not os.path.exists('outputs'):
            os.makedirs('outputs')
        torch.save(model.state_dict(), output_path)
        torch.save(model.state_dict(),f"{world_size}model_state_dict.pth")
        print(f"Training checkpoint saved at {output_path}")
